...
def embedding_layer(ids_, V, embed_dim, init_scale=0.001):
    ...
    W_embed_ = tf.get_variable(name="W_embed", shape=[V, embed_dim], dtype=tf.float32, initializer=tf.random_uniform_initializer(minval=-init_scale, maxval=init_scale))
...
def fully_connected_layers(h0_, hidden_dims, activation=tf.tanh,
    ...
    for i, hdim in enumerate(hidden_dims):
        ...
        if dropout_rate > 0:
            if is_training:
                h_ = tf.nn.dropout(h_, dropout_rate)
...
def softmax_output_layer(h_, labels_, num_classes):
    ...
    with tf.variable_scope("Logits"):
        ...
        W_out_rows = h_.get_shape()[1]
        W_out = tf.get_variable(name='W_out', shape=[W_out_rows, num_classes], dtype=tf.float32, initializer=tf.random_normal_initializer)
        b_out = tf.get_variable(name='b_out', shape=[num_classes], dtype=tf.float32, initializer=tf.zeros_initializer)
    ...
    with tf.name_scope("Softmax"):
        ...
        logistic_losses = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits_, labels=labels_)
        loss_ = tf.reduce_mean(logistic_losses, axis=0)
...
def BOW_encoder(ids_, ns_, V, embed_dim, hidden_dims, dropout_rate=0,
    ...
    h_ = fully_connected_layers(sum_embeddings, hidden_dims, dropout_rate=dropout_rate, is_training=is_training)
